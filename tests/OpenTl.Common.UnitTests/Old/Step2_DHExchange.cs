...
namespace OpenTl.Common.UnitTests.Old
{
    ...
    {
        public Step2Response FromBytes(byte[] response, byte[] _newNonce)
        {
        ...
        public byte[] ToBytes(byte[] nonce, byte[] serverNonce, byte[] newNounce, List<byte[]> fingerprints, BigInteger pq)
        {
            ...
            using (var pqInnerData = new MemoryStream(255))
            {
                ...
                using (var pqInnerDataWriter = new BinaryWriter(pqInnerData))
                {
                    ...
                    pqInnerDataWriter.Write(newNounce);
